...
function isString(value) {
  // throw new Error('Not implemented');
  if (typeof value === 'object' && !(value instanceof String)) return false;
...
function isPalindrome(str) {
  ...
  // eslint-disable-next-line prettier/prettier, array-callback-return
  str.toLowerCase().split('').reverse().forEach((item) => {
      if (item !== ' ' && item !== '?' && item !== '!' && item !== ',') {
        result.push(item);
      }
    });

  if (strArr.join('') === result.join('')) return true;
  return false;
}
